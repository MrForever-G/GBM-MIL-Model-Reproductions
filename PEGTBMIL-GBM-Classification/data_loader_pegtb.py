...
class PEGTBMILDataset(Dataset):
    # Binary setting: Mesenchymal vs Proneural
    def __init__(self, csv_path, pt_root, K=512):
        df = pd.read_csv(csv_path)

        # Only keep Mesenchymal / Proneural
    ...
    def _load_pt(self, slide_id):
        ...
        # 注意：这里必须是 "features" 和 "coords"
        feat = data["features"].float()      # [1, N, 768]
        coord = data["coords"].float()       # [1, N, 2]
